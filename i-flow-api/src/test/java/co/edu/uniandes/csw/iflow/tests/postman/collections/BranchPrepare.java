 ...
 * @author Asistente
 */
public class BranchPrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionBranch.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,BranchDetailDTO branchs,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(branchs);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(BranchDetailDTO branchs) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:branchs.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(branchs, null).toString().concat("\",\""))); 
           }
