...
public class ImagePrepare  {
   ...
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionImage.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,ImageDetailDTO images,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(images);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(ImageDetailDTO images) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:images.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(images, null).toString().concat("\",\""))); 
           }
