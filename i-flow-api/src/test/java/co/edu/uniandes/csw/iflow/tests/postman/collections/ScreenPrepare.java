 ...
 */
package co.edu.uniandes.csw.iflow.tests.postman.collections;
 ...
 * @author Asistente
 */
public class ScreenPrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionScreen.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,ScreenDetailDTO screens,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(screens);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(ScreenDetailDTO screens) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:screens.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(screens, null).toString().concat("\",\""))); 
           }
