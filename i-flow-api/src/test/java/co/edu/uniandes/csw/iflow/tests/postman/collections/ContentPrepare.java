 ...
 * @author Asistente
 */
public class ContentPrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionContent.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,ContentDetailDTO content,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(content);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(ContentDetailDTO content) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:content.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(content, null).toString().concat("\",\""))); 
           }
